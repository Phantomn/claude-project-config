...
- **대안**: 훅 스크립트는 `.claude/hooks/scripts/`에 배치, `settings.json`에서 `.claude/` 상대 경로로 참조. 훅은 프로젝트 루트를 cwd로 실행하므로 상대경로가 안전. `${CLAUDE_PROJECT_DIR}`은 훅 실행 환경에서 설정되지 않아 사용 불가
...
- **훅 스크립트 경로는 `.claude/` 상대경로로**: `settings.json` 훅에서 `~/.claude/` 전역 경로 사용 시 다른 환경 복사 후 파일 없음 오류 발생. `${CLAUDE_PROJECT_DIR}`은 훅 실행 환경에서 설정되지 않아 사용 불가. 훅은 프로젝트 루트를 cwd로 실행하므로 `.claude/` 상대경로가 안전하고 올바른 방법.
