...
> 평가 1단계(증거 수집)에서 참조하는 **탐지 경로 예시 목록** — ⑤ 멀티 런타임 조사(§1·§2·§4)와 ② 지식 저장소 탐지(§3)를 커버한다.
> 목적은 하나다: **사용자가 잘 쓰고 있는 런타임·저장소를 평가자가 경로를 몰라 놓치는 사고를 막는 것.** 하네스 지시 파일이 없는 옵시디언-only 사용자의 볼트도 §3으로 잡는다.
...
## 3. 지식 저장소 탐지 (Knowledge Repository Detection)

> 하네스 지시 파일(CLAUDE.md/AGENTS.md 등)이 없거나 저장소 경로를 참조하지 않는 사용자도 있다.
> 이 경우에도 지식 저장소(자격 요건 ①)를 결정론적으로 찾을 수 있도록, 주요 로컬 노트 도구의 자체 흔적을 스캔한다.
> 스캔 원칙은 동일: **있는 것만, 읽기 전용, 정량 사실(경로·개수·mtime)만 기록.**

| 도구 | 전역 레지스트리 (전체 저장소 목록) | 저장소 마커 | 활동 증거 |
|---|---|---|---|
| **Obsidian** | macOS `~/Library/Application Support/obsidian/obsidian.json`<br>Windows `%APPDATA%\obsidian\obsidian.json`<br>Linux `~/.config/obsidian/obsidian.json` (Flatpak은 `~/.var/app/md.obsidian.Obsidian/config/obsidian/`) | 볼트 루트의 `.obsidian/` 폴더 | `.obsidian/workspace.json`·`app.json` mtime |
| **Logseq** | `~/.logseq/` (graphs 캐시·설정) | 그래프 루트의 `logseq/` 폴더 | `logseq/` 하위 파일 mtime, `journals/` 최근 파일 |
| **Joplin** | `~/.config/joplin-desktop/` (DB `database.sqlite`) | — (DB 기반, 파일 볼트 없음) | DB 크기·mtime |
| **Dendron / Foam / 일반 md 레포** | — | 레포 루트의 `dendron.yml` / `.foam/` / 다수 `.md` + git | git log 최근 커밋 |
| **Notion / 클라우드 도구** | 로컬 흔적 없음 (데스크톱 앱 캐시는 비신뢰) | — | 사용자 문답 + 내보내기 폴더 유무로 확인 |

**Obsidian 판독법**: `obsidian.json`의 `vaults` 맵에 각 볼트의 절대 경로(`path`)·최종 오픈 타임스탬프(`ts`, ms)·현재 열림(`open`)이 있다. 가장 최근 `ts`(또는 `open: true`)인 볼트를 **주 볼트 후보**로 제시하고 사용자에게 확인받는다. `ts`가 30일 이내면 자격 요건 ③(최근 30일 사용 흔적)의 정량 증거가 된다.

```bash
# Obsidian 전체 볼트 나열 (macOS 예)
python3 -c "import json,pathlib; d=json.load(open(pathlib.Path.home()/'Library/Application Support/obsidian/obsidian.json')); [print(v['path'], v.get('ts'), v.get('open',False)) for v in d['vaults'].values()]"
```

탐지된 저장소는 **시스템 경계 선언에 포함할지 사용자에게 확인**한 뒤 아티팩트 #3(최상위 구조)·#4(frontmatter 스키마)·#5(검색 인덱스)·#7(인박스 규모) 조사 대상으로 삼는다. 사용자가 이미 저장소를 지목했다면 이 스캔은 교차 확인용이다(누락 볼트 발견).
...
| 지식 저장소 레지스트리 (`obsidian.json`, `~/.logseq/` 등) | 저장소 수·경로·최종 오픈 ts·마커 mtime | 자격 요건 ①③, 시스템 경계 선언, 아티팩트 #3~#7 조사 대상 식별 |
