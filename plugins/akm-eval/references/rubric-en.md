# AKM Index v1.2 — Agentic Knowledge Management Evaluation Framework
...
- **v1.2.0** (2026-08-02): Descriptive Dimensions layer — S Scale (7 fields) · A Autonomy (8 operations) · O Output coupling (90 days) reported alongside the score, plus the aggregate visualization grammar (2×2 matrix, bubbles, Edwards-Parry response surface). **Scoring (25 criteria, anchors, weights, formula) is identical to v1.1.0** — dimensions are norming context that never enters the score, so all prior scores remain comparable. Spec: [akm.cmdspace.work/dimensions.html](https://akm.cmdspace.work/dimensions.html) · downloads `akm-dimensions-v1.2-kr/-en.md`
