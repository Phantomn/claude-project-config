...
- **name**: always set one — that is what makes the agent addressable via `SendMessage({to: name})`
- ~~`mode`~~ / ~~`team_name`~~: **passing them is a no-op** (both "Deprecated; ignored" in the live
  `Agent` schema). Teammates inherit the parent session's permission mode.
